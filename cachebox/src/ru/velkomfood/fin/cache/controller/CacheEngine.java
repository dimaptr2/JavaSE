...
/**
 * Created by dpetrov on 11.01.2017.
 */
public class CacheEngine {
    ...
    public void buildReceipt() {
        ...
        while (itcj.hasNext()) {
            ...
            Iterator<DeliveryItem> itdlv = items.iterator();
            while (itdlv.hasNext()) {
                DeliveryItem di = itdlv.next();
                if (di.getDeliveryId() == cj.getDeliveryId()) {
    ...
    public Map<Long, BigDecimal> calculateTotalQuantityByDelivery(List<Receipt> rs) {
        ...
        while (iterator.hasNext()) {
            ...
            Double q = 0.000;
            for (ReceiptItem item: r.getItems()) {
                if (r.getDeliveryId() == item.getDeliveryId()) {
